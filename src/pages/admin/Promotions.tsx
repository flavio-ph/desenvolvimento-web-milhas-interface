    ...
    <div className="max-w-7xl mx-auto p-8 bg-white min-h-screen">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="p-2 bg-rose-50 rounded-lg"><Tag className="text-rose-500" size={24} /></div>
          <h1 className="text-2xl font-bold text-slate-800">Central de Promoções</h1>
        </div>
      </div>
      ...
      <div className="flex gap-8 border-b border-slate-100 mb-8">
        <button
          ...
          className={`pb-4 text-sm font-bold ${activeTab === 'cadastrar' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-400'}`}
        >
          {editingId ? 'Editando Promoção' : 'Cadastrar Nova'}
        </button>
        <button
          onClick={handleCancelEdit}
          className={`pb-4 text-sm font-bold ${activeTab === 'gerenciar' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-400'}`}
        >
          Gerenciar Ativas
        </button>
      </div>
      ...
      {activeTab === 'gerenciar' ? (
          ...
          {loading ? <div className="p-20 flex justify-center"><Loader2 className="animate-spin" /></div> : (
            <table className="w-full text-left">
              <thead className="bg-slate-50/50">
                <tr>
                  <th className="px-6 py-4 text-[11px] uppercase tracking-widest text-slate-400 font-bold">Promoção</th>
                  <th className="px-6 py-4 text-[11px] uppercase tracking-widest text-slate-400 font-bold text-center">Bônus</th>
                  <th className="px-6 py-4 text-[11px] uppercase tracking-widest text-slate-400 font-bold text-center">Expiração</th>
                  <th className="px-6 py-4 text-right text-[11px] uppercase tracking-widest text-slate-400 font-bold">Ações</th>
                </tr>
              </thead>
                ...
                {promotions.map((promo: any) => (
                  <tr key={promo.id} className="group hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-5">
                      <div className="text-[10px] uppercase font-bold text-indigo-500 mt-1">{promo.nomeProgramaPontos}</div>
                      <div className="font-bold text-slate-800 text-base">{promo.titulo}</div>
                      <p className="text-xs text-slate-400 leading-relaxed line-clamp-3">{promo.descricao || 'As regras aparecerão aqui...'} </p>
                    </td>
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={() => handleDelete(promo.id)} className="p-2 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start animate-fadeIn">
          <form onSubmit={handleSubmit} className="flex-1 space-y-5 w-full">
            <div className="flex justify-between items-center mb-2">
              ...
              {editingId && (
                <button
                  type="button"
                  onClick={handleCancelEdit}
                  className="px-4 py-2 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-xl transition-colors"
                >
                  Cancelar Edição
                </button>
              )}
            ...
            <button disabled={isSubmitting} className="w-full py-4 bg-indigo-600 text-white rounded-xl font-black flex items-center justify-center gap-2">
