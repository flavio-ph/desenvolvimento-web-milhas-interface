...
import {
  ...
  X, Check, Eye, Pencil, Trash2, Loader2
} from 'lucide-react';
  ...
  const carregarBandeiras = async () => {
    try {
      ...
      addToast({
        ...
        description: 'Não foi possível buscar as bandeiras.'
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      addToast({
        ...
        description: 'A bandeira foi removida com sucesso.'
      });
      ...
      addToast({
        ...
        description: 'Verifique se existem cartões vinculados a esta bandeira.'
      });
    } finally {
      setIsDeleting(false);
    ...
    try {
      ...
      if (editingId) {
        ...
        addToast({
          type: 'success',
          title: 'Sucesso',
          description: 'Nova bandeira cadastrada.'
        });
      }
      ...
      addToast({
        ...
        description: 'Não foi possível salvar a bandeira.'
      });
    }
  };
    ...
    <div className="space-y-8 animate-fadeIn relative pb-20 max-w-7xl mx-auto py-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
              <tr>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest text-center">Bandeira</th>
              ...
              {isLoading ? (
                ...
                brands.map((brand) => (
                  <tr key={brand.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    {/* COLUNA: AÇÕES (Centralizada e Visível) */}
                    <td className="px-6 py-5 text-center">
      ...
      {isModalOpen && (
        ...
        >
          ...
          {/* CONTAINER DO MODAL */}
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-5xl rounded-[32px] shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 animate-scaleIn">
            ...
            <div className="p-10 space-y-8 overflow-y-auto max-h-[90vh]">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsModalOpen(false)}>
                  <X className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200" />
                </button>
              </div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-3">
                    {PRESET_COLORS.map(c => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-xl ${c.class} flex items-center justify-center transition-all ${newBrand.colorObj.name === c.name
                ...
                <div>
                  ...
                  <div className="flex gap-4 mt-3">
                    <button
                      ...
                      className={`flex-1 py-4 rounded-xl font-bold transition-colors ${newBrand.status === 'ACTIVE'
                        ...
                        : 'bg-slate-50 text-slate-400 dark:bg-slate-800 dark:text-slate-500 border border-transparent'
                        }`}
                    >
                      Ativa
                    </button>
                    <button
                      ...
                      className={`flex-1 py-4 rounded-xl font-bold transition-colors ${newBrand.status === 'INACTIVE'
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-10 flex flex-col items-center justify-center gap-8 border-l border-slate-200 dark:border-slate-800 relative">
              ...
              {/* CARTÃO */}
              <div
                className={`w-full max-w-sm h-48 rounded-2xl shadow-2xl ${newBrand.colorObj.class} p-6 text-white flex flex-col justify-between transform hover:scale-105 transition-transform z-10`}
              >
                {/* TOPO */}
                ...
                <div className="flex flex-col gap-3">
                  <div className="w-10 h-6 rounded-md bg-yellow-400/90 shadow-sm"></div>
                  <div className="w-full h-2 rounded-full bg-white/30"></div>
                </div>
              </div>

              {/* TAG NA TABELA */}
              <div className="bg-white dark:bg-slate-900 px-6 py-4 rounded-xl shadow-lg border border-slate-100 dark:border-slate-800 text-sm font-bold w-full max-w-sm z-10">
                <p className="text-xs text-slate-400 dark:text-slate-500 mb-2 uppercase">Preview na Tabela</p>
                <div className="flex items-center gap-3">
                  <span className={`w-10 h-6 rounded flex items-center justify-center text-[8px] text-white font-bold ${newBrand.colorObj.class}`}>
                    {newBrand.nome ? newBrand.nome.substring(0, 4).toUpperCase() : 'AAAA'}
                  </span>
                  <span className="dark:text-white">{newBrand.nome || 'Nome da Bandeira'}</span>
                </div>
              </div>
